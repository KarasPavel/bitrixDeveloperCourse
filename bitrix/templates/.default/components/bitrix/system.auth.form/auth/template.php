...
<?endforeach?>
    ...
    <noindex><a href="<?=$arResult["AUTH_FORGOT_PASSWORD_URL"]?>" class="hd_forgotpassword" rel="nofollow"></noindex><?=GetMessage("AUTH_FORGOT_PASSWORD_2")?></a>

    <?if ($arResult["STORE_PASSWORD"] == "Y"):?>
    	<div class="head_remember_me" style="margin-top: 10px">
										<input id="USER_REMEMBER_frm" name="USER_REMEMBER" value="Y" type="checkbox">
										<label for="USER_REMEMBER_frm" title="<?=GetMessage("AUTH_REMEMBER_ME")?>"><?=GetMessage("AUTH_REMEMBER_ME")?></label>
									</div>
    <?endif?>
...
<?if($arResult["NEW_USER_REGISTRATION"] == "Y"):?>
        <noindex><a href="<?=$arResult["AUTH_REGISTER_URL"]?>" rel="nofollow" class="hd_signup"></noindex>Зарегистрироваться</a>
<?endif?>
...
?>
    ...
    <a href="?logout=yes" class="<?=$APPLICATION->GetCurPageParam("logout=yes",array(
            "login",
            "logout",
            "register",
            "forgot_password",
